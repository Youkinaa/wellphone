...
简单找不到按钮可局部再观察；预算耗尽后如果存在不同的可达路径，或遇到商家缺货、会议时间变化、用户修改目标，才进入全局 replan。设备或目标 App 仍不可用时按 DEVICE_UNAVAILABLE 暂停，不用 replan 反复重开。自动 replan 默认最多 2 次，用户明确修改单独计数；达到预算如实报告阻碍。

### 4.1 MVP 的有限重试

按用户最新取舍，美团专项崩溃/ANR 排查暂缓；执行时采用本契约定义的局部恢复路径处理偶发失败，不新增恢复框架。每个业务节点共用 `max_local_retries=2`（首次尝试之外最多两次恢复），仍受 `max_actions` 约束；派发前把计数保存到 RunStore，重连、进程恢复、换 attempt 或 replan 延续同一未完成目标时不重置额度。达到上限就报告受阻，不能靠新节点 ID 循环重跑。此处定义预期行为，尚无产品重试实现。

| 失败位置 | 允许的恢复 |
| --- | --- |
| 观察、找控件、已确认未生效的普通导航 | 重新观察当前状态，再选择允许的下一动作；不用旧图/旧元素重放点击，填写/加购也不能默认重复 |
| App 已退出，需要重新打开 | Android 与副屏仍有效、没有未决业务提交，且符合第 5 节同包任务隔离时，可通过既有 `phone.launch_app` 在副屏重开；先取新观察再继续。ANR 不直接当作进程已退出，不自动 force-stop、清数据或处理主屏弹框 |
| 保存会议/日历、下单、发送、付款后的超时或崩溃 | 先按第 7 节核查已发生的结果，不允许用普通重试路径再次提交；无法确认则 `OUTCOME_UNKNOWN` 暂停 |
| 整个 AVD 离线/退出 | 运行时进入 `DEVICE_UNAVAILABLE`，停止业务派发；在已授权的开发/维护窗口按 4.2 留证、初判后受控重启。重绑会话并核查在途操作后才能继续；业务模型没有整机重启工具 |

恢复仍经过现有版本、控制水位、授权和 display/epoch 检查，用户暂停/修改优先。新观察不能自动证明某次真实提交失败；重试是容错路径，不表示美团兼容性或全程无打扰已验收。

### 4.2 开发期的崩溃取证与受控重启

“暂停任务”不等于禁止开发者恢复模拟器。用户已授权的无人操作调试窗口内，由开发者/Coding Agent 先保存证据、做初步分层判断，再按需要进行一次受控重启与复跑；不要求先证明全部根因才恢复环境，也不把“没看到自己的异常”当作已排除代码问题。首版使用已有命令和本地日志，不新增 watchdog、诊断服务或模型可调用的整机重启工具。

1. **确认故障并留证。** 区分 App crash/ANR、Android/system_server 故障、宿主 QEMU 退出和单纯 ADB 断连。保存崩溃时点、最后一个工具/ADB 动作、代码版本、AVD/工具版本与启动参数、任务及未决操作引用；读取 emulator stdout/stderr、进程退出码、宿主 kernel/Apport/core，设备可读时再取相关 Android crash/ANR/tombstone 摘要。整机退出后先保留现有宿主证据，不为了等待已丢失的 logcat 无限阻塞。日志只作本地开发证据，不加入业务模型上下文。
2. **初判再恢复。** 确认进程已退出或整机失效才考虑重启；ADB 暂时断连不构成杀掉仍在使用的模拟器的理由。已定位为本项目逻辑错误时，先修正再验证；证据指向环境/上游或仍未知时，可保留“根因未定”结论后恢复一次。沿用已记录的配置与 userdata，启动检查通过后才重建副屏与新 epoch；同一故障复现就停止连续重启并补取证，不刷新 4.1 的动作预算。
3. **核查后复跑。** 在既有授权范围内，先尝试最小可重复步骤；需要归因时比较“Agent 不运行的手工/最小操作”和“相同条件加自动化动作”，逐步加入最后可疑动作。关闭 Agent 仍崩溃只说明 Agent 不是必要触发条件；仅开 Agent 时复现则沿调用链查，不能凭崩溃位于模拟器就宣布与本项目无关。未知业务提交先走第 7 节核查，不能从头重跑整段任务。

正式并发演示中模拟器崩溃意味着主屏已经被打断，该次“全程不打扰”验收失败；恢复环境后可以另起一次标明的新演示。正常使用中的 AVD 不因 Agent 卡住就被强制重启。上述开发恢复规则不恢复已暂停的美团专项研究优先级，只用于实际遇到故障时处理；本轮未实现自动恢复代码。
