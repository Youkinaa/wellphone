...
日期：2026-09-21。状态：**新增设计建议，待用户审阅；尚未实现 Web 应用或通用 Agent。** 用户已反馈美团和腾讯会议登录成功，并要求上传截图、输入自然语言的交互应用。本文件只定义新增交互层；执行语义继续以[运行时契约](2026-09-21-agent-runtime-contracts.md)为准。
...
这是客户端形态的建议，不把“用户说需要 app”解释成已确认必须原生或必须 Web。既有 LangGraph、LangChain、Redis、SQLite 和 GUI 工具选择不变。
...
此前主设计把“从副屏图库打开手机内图片”设为默认资料路径。新增上传需求后，建议 MVP 改为：
...
“整理手机图库里已有图片”保留为另一个可选输入场景，需图库/查看器通过副屏预检后才启用。没有通过时如实显示不支持，不将上传路径包装为图库读取能力。这一变更需与用户一并审阅；相关主设计和验收文档注明建议状态。
...
| `POST /conversations/{id}/messages` | 接收 `client_message_id,text,artifact_ids,target_task_id?`，持久接纳后返回 receipt |
...
| `POST /tasks/{id}/decisions` | 接收具体提案的确认/拒绝；绑定 `proposal_id,operation_id,canonical_args_hash` 与基准版本 |
...
1. **环境与页面验证继续推进。** 先验证美团硬件渲染配置，再在短暂无人操作窗口检查真实 App 的副屏启动、树、中文字段和跳转；真人并发输入另约时间。登录成功只是账号准备完成。
2. **交互层设计审阅后可独立开发。** 上传、消息持久接纳、状态查询、控制 receipt 与 SSE 重连可以用明确标注的测试运行时验证。真实设备未就绪时界面显示不可执行，不能模拟“已完成”。不必等所有 App 都通过才做前端。
