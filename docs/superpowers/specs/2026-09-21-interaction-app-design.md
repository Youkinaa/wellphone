...
1. **腾讯会议副屏预检先行，美团专项排查暂缓。** 按用户最新取舍，保持 4 核/6 GiB/host，不继续切换美团图形配置或定位 native crash；已知问题见[准备报告](../../validation/2026-09-21-real-app-preparation.md#33-用户要求升级资源4-核--6-gib-与新的失败证据)。先检查已登录腾讯会议的副屏启动、列表、预约页、中文字段和页面转移；真人并发输入在对应验收窗口检查。登录成功不代表这些页面已通过。
2. **交互层可按本设计独立开发。** 上传、消息持久接纳、状态查询、控制 receipt 与 SSE 重连可以用明确标注的测试运行时验证。真实设备未就绪时界面显示不可执行，不能模拟“已完成”。不必等所有 App 都通过才做前端。
3. **第一条闭环选腾讯会议。** 页面与隔离门槛通过后，接 LangGraph/LangChain、版本化 DAG、外置 skill 和受控工具：自然语言查询指定时段 → 读副屏列表 → 返回结果及证据；再扩展为按明确主题/时间预约 → 从列表重新打开核验。前者先验证完整数据通路，后者验证真实写入。若会议页面也受阻，再评估日历作为首条，不能事先写成已可用。
4. **补齐 MVP 并扩展演示。** 接通澄清/授权、运行中修改、暂停继续与未知结果恢复，再完成“上传行程图 → VLM → 日历”。美团作为后续第三条路径，偶发失败按[有限重试契约](2026-09-21-agent-runtime-contracts.md#41-mvp-的有限重试)处理；反复失败则报告受阻，不阻塞前两条和通用运行时，也不把准备订单写成已下单。
