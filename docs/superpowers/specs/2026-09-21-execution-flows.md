...
`PAUSED` 携带 `pause_id`、`pause_reason`、当前待办和继续条件。原因只保留六类：`CLARIFICATION`（缺信息）、`AUTHORIZATION`（缺授权）、`USER_TAKEOVER`（需要人工操作）、`USER_PAUSE`（用户主动暂停）、`DEVICE_UNAVAILABLE`（设备/显示不可用）、`OUTCOME_UNKNOWN`（真实提交结果未明）。结果未知不是 FAILED，等待用户也不因超时自动变成同意或执行。
...
| 设备断开、副屏无效、用户转到同包 App；Backend/Gateway 判定 | 停止设备写入，不降级主屏 | `DEVICE_UNAVAILABLE`：原因及最小处理方式 | 设备恢复/冲突解除，建立有效会话并取新观察 |
...
首版只需这一条恢复路径，不做任意时间回溯、自动退款/撤销、无限重试、跨任务补偿或多实例故障转移。正常路径与本页列出的中断分支是必要测试范围；不为假想组合堆积完整异常矩阵。
