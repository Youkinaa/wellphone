...
**新增交互层建议，待审阅：**响应式 React Web + FastAPI + SSE，支持上传截图、自然语言、暂停/继续/取消与结果核验。上传原图直接用于 VLM 提取；业务查询和写入仍使用副屏 GUI。客户端不维护独立执行器，所有控制进入既有 Coordinator/RunStore。具体界面范围、接口与对原图库入口的调整见[交互应用设计](2026-09-21-interaction-app-design.md)。
...
首版不部署单独 OCR 引擎。根据新增上传需求，建议将默认路径由“副屏图库打开图片后取帧”改为“用户上传原图后直接提取”，这项入口调整待用户审阅。模型适配器把输入 artifact 解析为 LangChain 图像消息及接口支持的实际图片载荷，输出符合 schema 的事件候选；只在 prompt 中写本地路径不算传图。行程 skill 提供领域解释规则；提取、校验和后续 GUI 操作仍复用通用运行时，不另建日历执行引擎。手机已有图库图片读取保留为另行验收的可选输入，不伪称上传图已经在手机上打开。
