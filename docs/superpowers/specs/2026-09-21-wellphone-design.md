...
新增交互需求后，建议将上传、消息接入、状态展示及其最小持久化作为可并行开发的独立部分，不等待三个 App 全部验收；上述前置门槛继续约束完整业务内核与真实设备执行。交互测试用例不能冒充设备可用或真实任务完成。

同一个 Android 实例提供两块显示区域：主屏由用户操作；Agent 的目标 App 在虚拟副屏运行。设备观察产生的截图、窗口树，以及触摸、滑动、填写和返回操作绑定副屏，模型没有主屏操作工具。用户主动上传的图片属于任务输入材料，与设备观察分别标记来源。
...
AVD（Android Virtual Device）是一台虚拟 Android 设备的配置与数据。真实 App 准备已发现原 AOSP x86_64 镜像不能安装 ARM64 官网 APK，现改用含 ARM64 翻译的官方 Google APIs API 34 镜像；用户已确认美团和腾讯会议登录成功，真实业务及副屏页面仍待验收。镜像、来源、性能排查与操作入口以[准备报告](../../validation/2026-09-21-real-app-preparation.md)为准，不将旧镜像实验结果直接视为新环境通过。
...
本文件负责总体选型与范围；[运行时契约](2026-09-21-agent-runtime-contracts.md)负责模块职责、触发条件和输入输出；[执行流程](2026-09-21-execution-flows.md)负责状态、时序与人工交接；[交互设计](2026-09-21-interaction-app-design.md)负责页面和 HTTP/SSE。它们共同构成开发依据，变更时同步相关消费者和验收；各类规则只在其权威位置定义。[AGENTS.md](../../../AGENTS.md)规定开发对照和维护方式。

MVP 部署在一台电脑：浏览器工作台 + 一个 Python 后端进程（FastAPI、Coordinator、LangGraph 及内部模块）+ 本地 Redis + SQLite/文件目录 + Appium/scrcpy 辅助进程 + 一个 AVD。模型通过已有 API 访问。图中的模块不是各自独立部署的微服务；不增加任务队列服务器。

```mermaid
flowchart TB
    U[用户消息 / 图片上传 / 修改任务] --> UI[Web 工作台]
    UI --> IN[请求接入与控制事件]
...
**交互层设计基线，待实现：**响应式 React Web + FastAPI + SSE，支持上传截图、自然语言、暂停/继续/取消与结果核验。上传原图直接用于 VLM 提取；业务查询和写入仍使用副屏 GUI。客户端不维护独立执行器，所有控制进入既有 Coordinator/RunStore。具体界面范围、接口与对原图库入口的调整见[交互应用设计](2026-09-21-interaction-app-design.md)。

### 2.2 MVP 必做与暂缓

| MVP 必做 | 首版暂缓 |
| --- | --- |
| 一个活动任务，经通用运行时与外置 skills 完成真实 GUI 闭环 | 多用户/多设备/多 worker、任务排队和并行设备动作 |
| 自然语言与图片输入、意图/槽位理解、版本化 DAG、修改和有限 replan | 可视化拖拽 DAG、任意流程编程、向量检索平台 |
| 消息历史与唯一执行状态、步骤间暂停/继续、重启后对账 | 任意历史节点回溯、自动退款/补偿、跨实例故障转移 |
| 副屏隔离、必要授权、未知提交禁止盲重试、可核验结果 | 通用风控引擎、多级审批、大量假想异常分支 |
| 正常路径及核心停止/授权/恢复边界测试，保留必要失败证据 | 镜像实现逻辑的测试、长期影子执行器、重复文档 |

日历、腾讯会议、外卖是同一能力体系下的三条目标演示，各自以真实页面实测为准。没有通过的路径显示受阻，不用假数据填充完成结果。先贯通一条已验证路径，再扩展 skill；与实际设备无关的接入/UI 可并行推进。
...
图片内内容、自绘区域、没有标签的图标、缺失或歧义严重的树才触发视觉观察。GUI 定位时 VLM 接收当前副屏图并提出目标，仍走同一观察新鲜度和显示守卫；它不能补出 App 未提供的中文填写能力。行程截图继续用 VLM 提取事件，新上传入口拟使用用户原图；上传图不是手机观察，不能用其中的坐标点击设备。截图可保留为本地证据，但无需每轮传给模型；实际延迟与 token 收益尚未测量。
...
| 场景 | 真实 GUI 操作 | 成功证据 |
| --- | --- | --- |
| 行程截图 → 日历 | 拟由 Web 上传原图、VLM 提取结构化事件，校验后进入副屏日历检查并创建；手机图库读取为可选场景 | 从日历列表重新打开详情核对字段；不调用 Calendar Provider 代写 |
...
最新实施顺序：用户决定暂缓美团专项崩溃/ANR 排查；优先预检腾讯会议并贯通“自然语言查询 → 副屏 GUI → 证据”，再扩展预约创建与行程截图转日历，Web 接入可并行开发。美团保留为第三条演示，偶发失败按[有限重试契约](2026-09-21-agent-runtime-contracts.md#41-mvp-的有限重试)处理；重试上限或未知提交仍会阻止继续操作。外卖演示若不实际提交，应明确称“按需求选餐并准备订单”；不能写“已点好外卖”。真实订单演示按具体授权和实际费用安排，不能用模拟页面冒充第三方 App。
...
首版不部署单独 OCR 引擎。根据新增上传需求，默认资料路径改为“用户上传原图后直接提取”。模型适配器把输入 artifact 解析为 LangChain 图像消息及接口支持的实际图片载荷，输出符合 schema 的事件候选；只在 prompt 中写本地路径不算传图。行程 skill 提供领域解释规则；提取、校验和后续 GUI 操作仍复用通用运行时，不另建日历执行引擎。手机已有图库图片读取保留为另行验收的可选输入，不伪称上传图已经在手机上打开。

```mermaid
flowchart LR
    IMG[用户上传原始行程图片] --> VLM[VLM 提取事件与证据]
...
小字先放大/裁切用户提供的原图，保留来源关系；可选的图库输入则在副屏放大后重拍。有限重试后仍不清楚就澄清；重复截图要合并候选，再查可见日历以避免重复创建。保存后重新打开日历详情，将实际字段与已确认候选比较。模型配置须先验证图像理解与结构化输出；只有实测准确率、成本或延迟需要时才评估独立 OCR。
...
首次设计阶段交付了设计、协议、来源与验证计划；此后已推进机制探针与 App 安装准备，当前进度以验证报告为准。通用 Agent 和交互应用仍未实现，不能把设计目录当作已运行的产品。
