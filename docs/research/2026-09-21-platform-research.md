...
**版本说明：**这是首轮平台研究，保留 iOS、系统数据接口与副屏的选型依据。用户随后选择“主屏给用户、副屏由通用 Agent 操作真实 App”，现行方案以 [V2 主设计](../superpowers/specs/2026-09-21-wellphone-design.md)为准。R1–R4 的 Provider 路线是未采用的历史候选，不进入当前 MVP；GUI 副屏已升为首版必过门槛。新的编排、窗口树、中文输入与 App 资料见[本轮复核](2026-09-21-agent-runtime-and-gui-research.md)。
...
**判断：**在该历史候选中，Provider 只处理有限操作、模型等待在电脑，可减少通知与后台调度复杂度。此判断不能直接套用于 V2 的无障碍桥与 scrcpy server；它们的存活、授权和断连恢复另行验证。若未来能力需要长时间手机计算，必须重新设计执行生命周期，不能无限占用一次 IPC。
