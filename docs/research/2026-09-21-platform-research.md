...
核验日期：2026-09-21。本篇为首轮历史资料研究，当时**没有设备或模型调用实验结果**。后续已经运行指定 AVD 的原生控件/合成 IME 并发探针，当前证据见[实测报告](../validation/2026-09-21-appium-concurrency-probe.md)，不要用本篇的历史环境状态代替现状。下文区分来源直接支持的事实、据此作出的工程判断，以及待验证项。

**版本说明：**这是首轮平台研究，保留 iOS、系统数据接口与副屏的选型依据。用户随后选择“主屏给用户、副屏由通用 Agent 操作真实 App”，现行方案以 [V2 主设计](../superpowers/specs/2026-09-21-wellphone-design.md)为准。R1–R4 的 Provider 路线是未采用的历史候选，不进入当前 MVP；GUI 副屏已升为首版必过门槛。新的编排、窗口树、中文输入与 App 资料见[本轮复核](2026-09-21-agent-runtime-and-gui-research.md)；其中[第 7 节](2026-09-21-agent-runtime-and-gui-research.md#7-复用手机自动化框架与-mcp)已改选 Appium + scrcpy + 薄适配层，替代默认自写整套无障碍桥的设想。
...
| --- | --- | --- | --- |
| Android 模拟器，单个 AVD | 开发 APK、ADB/无障碍 GUI 桥，验证同实例副屏操作 | 没有证明 OEM 真机行为，原题物理手机部署项未覆盖 | **主要开发与演示目标** |
...
## 2. Android：系统数据接口候选（历史方案，V2 不采用）
...
**判断：**电脑通过 ADB 请求手机端小型能力，无须先把 Companion 拉到前台；模型等待放在电脑，不需要手机常驻服务。这是首轮设计候选；V2 改为 GUI-only，不使用 Provider 代办业务。
...
**判断：**若采用数据接口路线，可直接写日历再重读 event ID 核验。V2 不采用这条路线，而从真实日历 App 创建并重新打开详情检查；UI 没有显示 event ID 时不能声称获取了它。
...
**判断：**在该历史候选中，Provider 只处理有限操作、模型等待在电脑，可减少通知与后台调度复杂度。此判断不能直接套用于 V2 的 Appium instrumentation 与 scrcpy server；它们的存活、授权和断连恢复另行验证。若未来能力需要长时间手机计算，必须重新设计执行生命周期，不能无限占用一次 IPC。
...
**本轮补充：**已继续追踪 Android 14 的显示、task 排序与 IME 请求路径，确认禁抢标志还通过 `mDontMoveToTop` 约束 task 置顶；非 top display 的普通 IME 请求在切换当前连接前被拒绝。因此标准控件直接写中文的系统依据已收敛，具体前提与固定 commit 见[本轮研究 2.3](2026-09-21-agent-runtime-and-gui-research.md)。剩余实验主要针对实际镜像、控件和 App 页面行为，不再把基础机制统称为未知。
...
目前只确认 `.env` 中存在模型配置变量，未输出其值，也未调用模型 API。V2 仍未验证：模拟器启动、模型能力、副屏 flags 和中文输入、商业 App 安装登录与跳屏、真实结果核验、任务时延及性能影响；真机行为也未知。Provider 的 UID/权限等实验已退出当前范围。[验证计划](../validation/2026-09-21-feasibility-and-demo.md)区分必过的模拟器 GUI 门槛和可选真机补验，不能从调研结论直接改写为产品能力。
